...
if(!place_empty(x+1,y)){
	check_ally = instance_place(x+1,y,objRebelSoldier);
	if(check_ally != noone){
		if(check_ally.sprite_index == sprRebelSoldierWait){
			sprite_index = sprRebelSoldierWait;
			hsp = 0;
		}
	}
	else{
		...
		if((check_enemyPlayer != noone && !check_enemyPlayer.enemy_is_royal) || check_enemySoldier != noone || check_enemyTower != noone){
			sprite_index = sprRebelSoldierAttack;
			hsp = 0;
		}
	}
}
else if(place_empty(x+1,y)){
	sprite_index = sprRebelSoldierRun;
}

enemy_spawn = instance_place(x, y, objRoyalSpawn);
...
if(sprite_index == sprRebelSoldierAttack && random_range(0,5) <= 2){
	if(floor(image_index) == 5) {
		CreateAttack(x, y, image_xscale, spr_SoldierHit, true_damage, obj_hitboxOfRebel, 0, dam_arr[0], chance, noone, 0);
	} 
}
